...
public class Model extends Observable {
    ...
    public static boolean maxTileExists(Board b) {
        ...
        for (int i = 0; i < boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                Tile boardNum = b.tile(i, j);
                if (boardNum == null) {
                    continue;
                }
                if (boardNum.value() == MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int i = 0; i < boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                Tile currentBoard = b.tile(i, j);
                if (i - 1 >= 0) {
                    Tile neigborBoard = b.tile(i - 1, j);
                    if (currentBoard.value() == neigborBoard.value()) {
                        return true;
                    }
                }
                if (i + 1 < boardSize) {
                ...
                if (j - 1 >= 0) {
                    Tile neigborBoard = b.tile(i, j - 1);
                    if (currentBoard.value() == neigborBoard.value()) {
                        return true;
                    }
                }
